...
module.exports = {
	...
	async createQuestionnarie(id,block) {
		...
		if(questionnarie.questions &&
			...
			qs.forEach(q => {
				let newQ = {};
				console.log(verbose('Pregunta ') + warn(j+1) + verbose(' tipo ') + warn(q.questType) + verbose('-------'));
				...
				if(q.questFooterShow) 	{newQ.footerShow	= q.questFooterShow;}
				if(q.questHelp)			{newQ.help = q.questHelp;}
				if(q.questDisplay)  {newQ.display = q.questDisplay;}
				if(q.answers &&
					Array.isArray(q.answers) &&
					q.answers.length > 0) {
					console.log(verbose('Generando respuestas'));
					let newAns = [];
					q.answers.forEach(a => {
						let ans = {
							type: a.ansType,
				...
				if(q.options &&
					...
					q.options.length > 0) {
					console.log(verbose('Generando opciones'));
					let newOpts = [];
					q.options.forEach(o => {
						newOpts.push({
							name	: o.optName,
				...
				if(q.group &&
					...
					q.group.length > 0) {
					console.log(verbose('Generando grupo'));
					let newGroup = [];
					q.group.forEach(g => {
						newGroup.push(g);
					});
		...
		if(task.items &&
			...
			task.items.forEach(item => {
				items.push({
					...
					w: item.itemW,
					notifications: item.itemNotifications
				});
			});
		}
		var options = {
